        ...
        child: Column(
          ...
          children: <Widget>[
            const ListTile(
              title: Text('Exportar'),
              subtitle: Text(
                'Los archivos se guardan sin guías ni piel de cebolla.',
              ),
            ),
            ListTile(
            ...
            ListTile(
              ...
              subtitle: Text(
                '${controller.project.frameCount} fotogramas numerados',
              ),
            ...
            ListTile(
              ...
              subtitle: Text(
                '${controller.project.frameCount} fotogramas a ${controller.project.fps} FPS',
              ),
            ...
            ListTile(
              ...
              subtitle: const Text(
                'Abre el menú para enviarlo a otra aplicación',
              ),
            ...
            ListTile(
              ...
              subtitle: const Text(
                'Genera la animación y abre el menú de Android',
              ),
      ...
      return Container(
        ...
        child: Row(
          children: <Widget>[
            ...
            if (compact)
              IconButton(
                onPressed: () => _showMobilePanel(context),
  ...
  Future<void> _save(BuildContext context) async {
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Proyecto guardado en $path')));
      }
    } catch (error) {
      if (context.mounted) {
        ScaffoldMessenger.of(
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(
      children: <Widget>[
        ...
        Text(
          controller.hasUnsavedChanges ? 'Cambios sin guardar' : 'Guardado',
          style: TextStyle(
            fontSize: 12,
            color: controller.hasUnsavedChanges
                ? Colors.amberAccent
...
class _PropertiesPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: ListView(
      children: <Widget>[
              ...
              children: <Widget>[
                ...
                DropdownButtonFormField<BrushPreset>(
                  ...
                  decoration: const InputDecoration(
                    labelText: 'Ajuste rápido',
                    isDense: true,
                  ),
                  items: const <DropdownMenuItem<BrushPreset>>[
                    DropdownMenuItem(
                      value: BrushPreset.pencil,
                      child: Text('Lápiz'),
                    ),
                    DropdownMenuItem(
                      value: BrushPreset.ink,
                      child: Text('Tinta'),
                    ),
                    DropdownMenuItem(
                      value: BrushPreset.marker,
                      child: Text('Marcador'),
                    ),
                  ],
                  onChanged: (value) {
                ...
                Row(
                  children: <Widget>[
                    ...
                    Expanded(
                      child: Text('Tamaño ${controller.brushSize.round()} px'),
                    ),
                  ],
                ),
                Slider(
        ...
        Padding(
          ...
          child: Row(
            children: <Widget>[
              ...
              IconButton(
                onPressed: controller.activeLayer > 0
                    ? controller.moveActiveLayerUp
                    : null,
              ...
              IconButton(
                onPressed:
                    controller.activeLayer <
                        controller.project.layers.length - 1
                    ? controller.moveActiveLayerDown
                    : null,
                icon: const Icon(Icons.arrow_downward_rounded),
                tooltip: 'Bajar capa',
              ),
              IconButton(
                onPressed: controller.project.layers.length > 1
                    ? () => _deleteLayer(context)
                    : null,
                icon: const Icon(Icons.delete_outline_rounded),
            ...
            itemBuilder: (context, index) {
              ...
              return Material(
                ...
                child: ListTile(
                  ...
                  leading: IconButton(
                    onPressed: () => controller.toggleLayerVisibility(index),
                    icon: Icon(
                      layer.visible
                  ...
                  trailing: IconButton(
                    onPressed: () => controller.toggleLayerLock(index),
                    icon: Icon(
                      layer.locked
                          ? Icons.lock_rounded
        ...
        Padding(
          ...
          child: Row(
            children: <Widget>[
              Expanded(
                child: Text(
                  'Opacidad de ${controller.layer.name}',
                  overflow: TextOverflow.ellipsis,
                ),
              ),
        ...
        Slider(
          value: controller.layer.opacity,
          min: 0,
          max: 1,
          onChanged: controller.setActiveLayerOpacity,
        ),
        Material(
          color: Colors.transparent,
          child: SwitchListTile(
            title: const Text('Papel cebolla'),
            value: controller.onionSkin,
            onChanged: controller.setOnionSkin,
          ),
        ),
        Material(
          color: Colors.transparent,
          child: SwitchListTile(
            ...
            value: controller.gridEnabled,
            onChanged: controller.setGridEnabled,
          ),
        ),
      ],
    ),
  ...
  void _deleteLayer(BuildContext context) {
    if (!controller.deleteActiveLayer()) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('El proyecto debe conservar al menos una capa.'),
        ),
      );
    }
  }
...
class _Timeline extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: const BoxDecoration(
      color: VelyntoraColors.surface,
      border: Border(top: BorderSide(color: VelyntoraColors.border)),
    ),
    child: Column(
      children: <Widget>[
        SizedBox(
          ...
          child: Row(
            children: <Widget>[
              ...
              IconButton(
                onPressed: () =>
                    controller.selectFrame(controller.activeFrame - 1),
              ...
              const SizedBox(width: 12),
              Text(
                '${controller.activeFrame + 1} / ${controller.project.frameCount}',
              ),
              const SizedBox(width: 14),
              TextButton(
              ...
              if (!compact)
                TextButton.icon(
                  onPressed: () => controller.addFrame(duplicate: true),
  ...
  void _deleteFrame(BuildContext context) {
    if (!controller.deleteFrame()) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('El proyecto debe conservar al menos un fotograma.'),
        ),
      );
    }
  }
            ...
            child: Column(
              ...
              children: <Widget>[
                for (final value in const <int>[6, 12, 15, 24, 30, 60])
                  RadioListTile<int>(value: value, title: Text('$value FPS')),
              ],
            ),
          ),
        ],
...
class _StatusBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(
      children: <Widget>[
        Text(
          '${controller.project.width} × ${controller.project.height}',
